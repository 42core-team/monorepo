...
Game::Game(const GameConfig& config) : config_(config), nextEntityId_(1) {
    // For simplicity, create two cores for two teams.
    // Team 1 core at (1000, 1000), Team 2 core at (9000, 9000)
                ...
                if (msg.contains("actions")) {
                    for (auto& actJson : msg["actions"]) {
                        ...
                        if (action.type == ActionType::Create) {
                            ...
                            // Expect data: {"unitId": X, "dx": D, "dy": D}
