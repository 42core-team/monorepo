...
void HardcodedWorldGenerator::generateWorld(uint64_t seed)
{
	...
	configWorldData.erase(std::remove_if(configWorldData.begin(), configWorldData.end(),
										 [](char c) { return c == '\n' || c == '\r'; }),
						  configWorldData.end());
	...
	for (size_t i = 0; i < configWorldData.size(); i++)
	{
		...
		{
			Logger::LogWarn("HardcodedWorldGenerator - Invalid obj at position (" + std::to_string(x) + ", " +
							std::to_string(y) + ") in map data.");
			continue;
		}
		...
		for (size_t cp = 0; cp < Config::game().corePositions.size(); cp++)
		{
			if (Config::game().corePositions[cp].x == static_cast<int>(x) &&
				Config::game().corePositions[cp].y == static_cast<int>(y) && configWorldData[i] != ' ')
			{
				Logger::LogWarn("HardcodedWorldGenerator - Found objects overlapping a core position at (" +
