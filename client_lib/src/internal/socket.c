...
int core_internal_socket_send(int socket_fd, const char *msg)
{
	...
	if (status_send < 0) perror("Socket send failed"), exit(EXIT_FAILURE);
...
static char *core_static_socket_readLine(int fd)
{
	...
	while (!(nl = buf ? strchr(buf, '\n') : NULL) && (r = read(fd, tmp, BUFFER_SIZE)) > 0)
	{
		tmp[r] = '\0';
		size_t old = buf ? strlen(buf) : 0;
		char *ext = realloc(buf, old + r + 1);
		if (!ext) return (free(buf), buf = NULL, NULL);
		buf = ext;
		memcpy(buf + old, tmp, r + 1);
	}
	if (!buf || !*buf) return (free(buf), buf = NULL, NULL);
	nl = strchr(buf, '\n');
